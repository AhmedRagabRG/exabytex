...
export async function generateMetadata({ params }: BlogPostPageProps): Promise<Metadata> {
  try {
    ...
    return {
      title: `${post.title} - مدونة Exa Bytex`,
...
export default async function BlogPostPage({ params }: BlogPostPageProps) {
  try {
    const { slug } = await params
    console.log('🚀 تحميل صفحة المقال لـ slug:', slug)
    ...
    console.log('✅ تم تحميل المقال بنجاح:', post.title)
              ...
              <div className="flex flex-wrap items-center gap-6 text-sm text-gray-500 border-t border-gray-200 pt-6 mb-8">
                <div className="flex items-center space-x-3 rtl:space-x-reverse">
                  ...
                  <div>
                    ...
                    <div className="text-xs text-gray-500">مؤلف</div>
                  </div>
                </div>
                
                <div className="flex items-center space-x-2 rtl:space-x-reverse">
                  <Calendar className="h-4 w-4 text-blue-500" />
                  <span>{post.publishedAt.toLocaleDateString('ar-EG')}</span>
                </div>
    ...
    console.error('💥 خطأ في صفحة المقال:', error)
    return (
      <div className="min-h-screen bg-white flex items-center justify-center">
        <div className="text-center">
          <h1 className="text-2xl font-bold text-red-600 mb-4">خطأ في تحميل المقال</h1>
          <p className="text-gray-600 mb-4">حدث خطأ أثناء تحميل المقال</p>
          <Link 
            href="/blog"
